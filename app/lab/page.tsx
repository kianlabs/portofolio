...
export default function LabPage() {
  return (
    <PageTransition>
      <div style={{ padding: "20px 20px", maxWidth: "900px", margin: "0 auto", width: "100%" }}>
        <div style={{ marginBottom: "14px" }}>
          <span style={{ fontSize: "13px", letterSpacing: "0.14em", color: "var(--text-dim)" }}>DIRECTORY / LAB</span>
        </div>

        <div style={{ marginBottom: "14px" }}>
          <h1 style={{ fontSize: "18px", fontWeight: 700, letterSpacing: "0.12em", color: "var(--text-main)", marginBottom: "4px" }}>
            KYAN LABORATORY
          </h1>
          <p style={{ fontSize: "14px", color: "var(--text-dim)", letterSpacing: "0.1em" }}>EXPERIMENTAL NODES</p>
        </div>

        {/* note */}
        <div style={{ border: "1px solid var(--border)", padding: "12px 16px", marginBottom: "14px", backgroundColor: "var(--bg-secondary)" }}>
          <p style={{ fontSize: "14px", color: "var(--text-sec)", lineHeight: "1.7" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
          {labItems.map((item, i) => (
            ...
            >
              <span style={{ fontSize: "13px", color: "var(--text-dim)", letterSpacing: "0.06em", paddingTop: "2px" }}>
                [{String(item.index).padStart(2, "0")}]
              </span>
              <div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "10px", marginBottom: "4px", flexWrap: "wrap" }}>
                  <h2 style={{ fontSize: "18px", fontWeight: 600, letterSpacing: "0.04em", color: "var(--text-main)", textTransform: "lowercase" }}>
                    {item.name}
                  </h2>
                ...
                <p style={{ fontSize: "15px", color: "var(--text-sec)", lineHeight: "1.6", maxWidth: "560px" }}>{item.desc}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" }}>
                <span style={{ fontSize: "13px", color: "var(--text-dim)", letterSpacing: "0.06em" }}>{item.year}</span>
                {item.url && (
